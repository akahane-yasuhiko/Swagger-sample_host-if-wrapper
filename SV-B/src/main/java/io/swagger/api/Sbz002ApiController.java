...
public class Sbz002ApiController implements Sbz002Api {
    ...
    public ResponseEntity<Sbz002bRes> sbz002(@Parameter(in = ParameterIn.DEFAULT, description = "description here", required=true, schema=@Schema()) @Valid @RequestBody Sbz002bReq body) {
        ...
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Sbz002bRes>(objectMapper.readValue("{\n  \"statusCode\" : \"statusCode\"\n}", Sbz002bRes.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<Sbz002bRes>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<Sbz002bRes>(HttpStatus.NOT_IMPLEMENTED);
    }
