...
public class Sbz003ApiController implements Sbz003Api {
    ...
    public ResponseEntity<Sbz003cRes> sbz003(@Parameter(in = ParameterIn.DEFAULT, description = "description here", required=true, schema=@Schema()) @Valid @RequestBody Sbz003cReq body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<Sbz003cRes>(objectMapper.readValue("{\n  \"statusCode\" : \"1\"\n}", Sbz003cRes.class), HttpStatus.NOT_IMPLEMENTED);
            } catch (IOException e) {
                log.error("Couldn't serialize response for content type application/json", e);
                return new ResponseEntity<Sbz003cRes>(HttpStatus.INTERNAL_SERVER_ERROR);
            }
        }

        return new ResponseEntity<Sbz003cRes>(HttpStatus.NOT_IMPLEMENTED);
    }
